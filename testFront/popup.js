...
document.addEventListener("DOMContentLoaded", async () => {
    // Check if recording is active and restore UI state
    const result = await chrome.storage.local.get(['isRecording', 'jobId']);
    if (result.isRecording) {
        updateUIForRecording(true);
        showStatusMessage("Recording in progress...", false);
    } else {
        updateUIForRecording(false);
    }
...
startButton.addEventListener("click", async () => {
    ...
    try {
        ...
        // Initialize RecordingService and start session
        const recordingService = new RecordingService();
        const result = await recordingService.startRecordingSession();
        
        if (result.success) {
            // Store recording service in chrome storage for access across popup instances
            ...
            updateUIForRecording(true);
            showStatusMessage("Continuous recording started with parallel uploads", false);
        } else {
            showStatusMessage("Failed to start recording: " + result.error, true);
            updateUIForRecording(false);
        }
...
stopButton.addEventListener("click", async () => {
    ...
    try {
        const recordingService = new RecordingService();
        const result = await recordingService.stopRecordingSession();
        ...
        if (result.success) {
            ...
            updateUIForRecording(false);
            showStatusMessage("Recording session stopped successfully", false);
        } else {
            showStatusMessage("Failed to stop recording: " + result.error, true);
        }
        
    } catch (error) {
        showStatusMessage("Error stopping recording: " + error.message, true);
    }
});
