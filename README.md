...
| Two identical runs, nothing changed, differ by 12.2 points of claim coverage and disagree on 9 of 24 verdicts. | This is the noise floor, measured on the model that ships. Any result smaller than it is resampling, and several earlier conclusions were. |
| Changing the answering model is worth 15.1 points and clears the floor. A human transcript (+8.3), corrected speaker attribution (+0.8) and supplied speaker roles (+1.4) do not. | The model is the only lever measured to move answer quality. The audio-side work has not reached the answer. |
...
That 66.6% is the `oracle` arm — the **human** AMI reference transcript, which exists to price our own transcription errors — not our transcript pasted whole. Measured separately, our own transcript pasted whole scores 65.1% on the same model and questions, 1.5 points from the human one with the same number of fully correct answers. Read carefully, that says transcription quality is not what is costing us. It is not yet a settled claim: the two arms were graded in different judge invocations, and this project's own rule is that such numbers are not comparable until re-graded together. Pasting a whole meeting also does not scale past a short one on any normal-context model.
